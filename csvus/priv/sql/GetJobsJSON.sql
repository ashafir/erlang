...
;
--------------------------------------------------------------------------------
-- drop function getjobsjson(int, text, text, float, float, int)
-- ;
create or replace function
GetJobsJSON(
        orgid   int,
...
and     case $2 
        when 'all' then true
        when 'allexports' then interfaceactiontype not in ('import')
        else $2 = interfaceactiontype
        end
and     $3 in ('all', status)
and     datecreated between _ts($4) and _ts($5)
order   by jobid desc
limit   $6;
$$ language sql
;
